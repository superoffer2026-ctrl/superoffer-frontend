...
@Component({
  ...
  styles: [`
    ...
    .links{display:flex;align-items:center;gap:30px;margin:auto}.links a{position:relative;padding:31px 0;color:#40505a;text-decoration:none;font-size:15px;font-weight:750}.links a:after{content:"";position:absolute;left:0;right:0;bottom:22px;height:2px;background:#ec641b;transform:scaleX(0);transition:transform .25s}.links a:hover:after,.links a.active:after{transform:scaleX(1)}
    .actions{display:flex;align-items:center;gap:26px}.actions a{text-decoration:none;font-size:15px;font-weight:800}.login{color:#2d2722}.start{padding:13px 21px;border-radius:99px;color:#fff;background:#25201c}
    .menu{display:none;margin-left:auto;width:44px;height:44px;border:1px solid #d9d0c8;border-radius:12px;background:#fff}.menu span{display:block;width:18px;height:2px;margin:5px auto;background:#29201b}.drawer{display:none;position:fixed;top:76px;left:14px;right:14px;padding:14px;border:1px solid #ded2c7;border-radius:16px;background:#fffaf5;box-shadow:0 25px 60px #38271c24}.drawer.open{display:grid}.drawer a{padding:13px;border-radius:9px;color:#352a24;text-decoration:none;font-size:16px;font-weight:700}.drawer a.start{text-align:center;color:#fff}
    @media(max-width:980px){.links,.actions{display:none}.menu{display:block}.brand{font-size:23px}}
  `],
  template: `
    <nav class="nav" [class.scrolled]="scrolled" aria-label="Primary navigation">
      ...
      <div class="links"><a routerLink="/students" routerLinkActive="active">Student</a><a routerLink="/organization" routerLinkActive="active">Organizations</a></div>
    ...
    <div class="drawer" [class.open]="menuOpen"><a routerLink="/students" (click)="menuOpen=false">Student</a><a routerLink="/organization" (click)="menuOpen=false">Organizations</a><a [routerLink]="['/auth/login',context]" (click)="menuOpen=false">Log in</a><a class="start" [routerLink]="['/auth/register',context]" (click)="menuOpen=false">Sign up</a></div>
  `
})
export class SiteHeaderComponent {
  @Input() context: 'student'|'organization' = 'student';
  menuOpen = false;
  scrolled = false;

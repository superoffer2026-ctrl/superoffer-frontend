        ...
        <aside class="offer-mailbox">
          <header class="mailbox-toolbar">
            ...
            <div class="compact-offer-filters">
              ...
              <button [class.active]="filter==='Accepted'" (click)="filter='Accepted'">Accepted <b>{{count('Accepted')}}</b></button>
              <button [class.active]="filter==='Shortlisted'" (click)="filter='Shortlisted'">Shortlisted <b>{{count('Shortlisted')}}</b></button>
              <button [class.active]="filter==='Rejected'" (click)="filter='Rejected'">Rejected <b>{{count('Rejected')}}</b></button>
            </div>
          </header>
...
export class StudentOfferInboxComponent {
  ...
  get filteredOffers(): StudentOffer[] {
    return this.offers.filter(offer => this.filter === 'All' || offer.status === this.filter);
  }
  ...
  count(status: OfferDecisionStatus) { return this.offers.filter(offer => offer.status === status).length; }
